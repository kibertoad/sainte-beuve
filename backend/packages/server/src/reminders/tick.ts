...
export async function runReminderTick(
  ...
  const total: TickResult = { sent: 0, failed: 0, skipped: 0, sessionsSwept: 0, aiReviewsPolled: 0 }
  for (const orgId of await tenancies(container)) {
    ...
    total.aiReviewsPolled += pass.aiReviewsPolled
  }
  return total
}
...
async function tickOrg(container: AppContainer, batchSize: number): Promise<TickResult> {
  ...
  result.aiReviewsPolled = await pollAiReviews(container, batchSize)
  if (due.length > 0 || result.sessionsSwept > 0 || result.aiReviewsPolled > 0) {
 ...
 * spend one tick's whole budget here and send no reminder at all. The oldest are
 * read first, so what is left over is picked up next tick rather than starved.
 *
